...
gen citescore=8.7 if title=="ACTUAL VIS-A-VIS PERCEIVED RISK OF FLOOD PRONE URBAN COMMUNITIES IN PAKISTAN" 
replace citescore=7.5 if title=="RISK PERCEPTION, EXPERIENCE, AND OBJECTIVE RISK: A CROSS-NATIONAL STUDY WITH EUROPEAN EMERGENCY SURVIVORS" 
replace citescore=7.5 if title=="EMPIRICAL ANALYSIS OF FARMERS' DROUGHT RISK PERCEPTION: OBJECTIVE FACTORS, PERSONAL CIRCUMSTANCES, AND SOCIAL INFLUENCE" 
replace citescore=11.5 if title=="HEALTH RISK PERCEPTIONS AND LOCAL KNOWLEDGE OF WATER-RELATED INFECTIOUS DISEASE EXPOSURE AMONG KENYAN WETLAND COMMUNITIES" 
replace citescore=2.5 if title=="INDIVIDUAL ACTUAL OR PERCEIVED PROPERTY FLOOD RISK: DID IT PREDICT EVACUATION FROM HURRICANE ISABEL IN NORTH CAROLINA, 2003?" 
replace citescore=7.3 if title=="ASSESSING DISASTER PREPAREDNESS AMONG LATINO MIGRANT AND SEASONAL FARMWORKERS IN EASTERN NORTH CAROLINA" 
replace citescore=2.4 if title=="RELUCTANCE OF PARAMEDICS AND EMERGENCY MEDICAL TECHNICIANS TO PERFORM MOUTH-TO-MOUTH RESUSCITATION" 
replace citescore=10.2 if title=="PERCEIVED AND PROJECTED FLOOD RISK AND ADAPTATION IN COASTAL SOUTHEAST QUEENSLAND, AUSTRALIA" 
replace citescore=8.7 if title=="THE BETTER THE BOND, THE BETTER WE COPE. THE EFFECTS OF PLACE ATTACHMENT INTENSITY AND PLACE ATTACHMENT STYLES ON THE LINK BETWEEN PERCEPTION OF RISK AND EMOTIONAL AND BEHAVIORAL COPING" 
replace citescore=6.6 if title=="EXPLORING A SPATIAL STATISTICAL APPROACH TO QUANTIFY FLOOD RISK PERCEPTION USING COGNITIVE MAPS" 
replace citescore=7.5 if title=="GEOGRAPHIC DISTRIBUTIONS OF EXTREME WEATHER RISK PERCEPTIONS IN THE UNITED STATES" 
replace citescore=7.5 if title=="INSIGHTS INTO FLOOD RISK MISPERCEPTIONS OF HOMEOWNERS IN THE DUTCH RIVER DELTA" 
replace citescore=8.4 if title=="THE ASSOCIATION BETWEEN ACTUAL AND PERCEIVED FLOOD RISK AND EVACUATION FROM HURRICANE IRENE, BEAUFORT COUNTY, NORTH CAROLINA" 
replace citescore=12.2 if title=="RISK PERCEPTION AND EMERGENCY EXPERIENCE: COMPARING A REPRESENTATIVE GERMAN SAMPLE WITH GERMAN EMERGENCY SURVIVORS" 
replace citescore=2.2 if title=="EXPLORING GENDER DIFFERENCES IN PREP INTEREST AMONG INDIVIDUALS TESTING HIV NEGATIVE IN AN URBAN EMERGENCY DEPARTMENT" 
replace citescore=8.8 if title=="PAST AND FUTURE WATER CONFLICTS IN THE UPPER KLAMATH BASIN: AN ECONOMIC APPRAISAL" 
replace citescore=6.6 if title=="PERCEIVED AND ACTUAL RISKS OF DROUGHT: HOUSEHOLD AND EXPERT VIEWS FROM THE LOWER TEESTA RIVER BASIN OF NORTHERN BANGLADESH" 
replace citescore=6.8 if title=="OBJECTIVE AND PERCEIVED RISK IN SEISMIC VULNERABILITY ASSESSMENT AT AN URBAN SCALE" 
qui save "${gsdOutput}\Top_papers_info.dta", replace
...
qui save "${gsdOutput}\Scopus_exported_refine_values.dta", replace
